...
function SignInFormComponent() {
  ...
  useEffect(() => {
    if (authActionInitiated && !isUserLoading && user?.uid) {
        ...
        const isAdmin = user.claims?.admin === true || user.role === 'superadmin' || user.role === 'admin' || user.declaredPosition === 'admin' || user.email === 'mkoton100@gmail.com' || user.email === 'beyondtransport@gmail.com' || user.email === 'michael@logisticsflow.co.za';
  ...
  const onSubmit = async (values: SignInFormValues) => {
    ...
    try {
      ...
      // Background profile verification (non-blocking)
      fetch('/api/checkAndCreateUser', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${idToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      }).catch(err => {
        console.warn("Background check notice:", err);
      });
