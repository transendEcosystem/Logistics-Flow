...
function ISADialog({ open, onOpenChange, partner, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; partner?: any; onSave: () => void; }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      else form.reset({ firstName: '', lastName: '', email: '', phone: '', mobile: '', contactPerson: '', companyName: '', status: 'new', type: 'isa', website: '', notes: '', address: '' });
    }
  }, [open, partner, form]);
  ...
  const fetchData = useCallback(async (limit: number = 20000) => {
    ...
    try {
        ...
        const [res, staffRes] = await Promise.all([
          performAdminAction(token, 'searchRegistry', { type: 'isa', term: searchTerm, outreachFilter, limit }),
      ...
      <div className="space-y-6 text-left text-foreground">
          <CardHeader className="px-0 pt-0 flex flex-col md:flex-row md:items-center justify-between gap-4 text-left text-foreground">
              ...
              <div className="flex gap-2 text-left text-foreground">
                  ...
                  <Button variant="outline" onClick={() => downloadDataAsCSV(filteredRecords, 'isa-backup.csv')} disabled={isLoading} className="text-left text-foreground"><Download className="mr-2 h-4 w-4"/>Export CSV</Button>
                  <BulkImportDialog type="isa" onComplete={() => fetchData()}><Button variant="outline" className="text-left text-foreground"><Upload className="mr-2 h-4 w-4" /> Import</Button></BulkImportDialog>
          ...
          <Card className="text-left text-foreground text-foreground">
              <CardContent className="pt-6 text-left text-foreground text-foreground">
                  <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6 p-4 bg-muted/30 rounded-lg text-left text-foreground text-foreground">
                      ...
                      <div className="space-y-1 text-left text-foreground">
                          <TagSelector
                              ...
                              onValueChange={(selectedTag) => {
                                  ...
                                  console.log('Selected tag:', selectedTag, 'registry:', 'isa');
                              }}
                          />
                      </div>
                      <div className="md:col-span-2 flex items-end gap-2 text-left">
                          <div className="flex-1 space-y-1 text-left">
                              <Label className="text-xs font-bold uppercase text-muted-foreground flex items-center gap-1.5 text-left"><Search className="h-3 w-3"/> Search Agency</Label>
                              <Input placeholder="Search criteria..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} onKeyDown={e => e.key === 'Enter' && fetchData()} className="h-9 bg-white" />
                          </div>
                      </div>
                  </div>
