          ...
          <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-8 py-4 max-h-[85vh] overflow-y-auto pr-2 text-left text-foreground">
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="website" render={({ field }) => ( <FormItem className="text-left text-foreground"><FormLabel>Website URL</FormLabel><FormControl><Input {...field} value={field.value || ''} className="bg-white border-2" placeholder="e.g. jhtrucking.co.za" /></FormControl></FormItem> )} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left text-foreground">
                    ...
                    <div className={cn("space-y-4 p-6 rounded-2xl border transition-all text-left text-foreground", form.watch('primaryContactRole') === 'ceo' ? "bg-primary/5 border-primary shadow-md" : "bg-slate-50 border-slate-200 shadow-inner")}>
                        <div className="flex items-center justify-between">
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-600 flex items-center gap-2 text-left text-foreground">
    ...
    try {
        ...
        if (token) {
          try {
            const [res, staffRes] = await Promise.all([
              performAdminAction(token, 'searchRegistry', { type, term: searchTerm, limit }).catch(() => ({ data: [] })),
        ...
        if (records.length === 0) {
          try {
            ...
            const q1 = query(collection(clientDb, 'partners'), where('type', 'in', ['transporter', 'transporters', 'haulier', 'hauliers']), limitFn(limit));
            const snap1 = await getDocs(q1);
            let list = snap1.docs.map(doc => ({ id: doc.id, ...doc.data(), source: 'Member' }));

            if (list.length === 0) {
              const q2 = query(collection(clientDb, 'leads'), where('type', 'in', ['transporter', 'transporters', 'haulier', 'hauliers']), limitFn(limit));
            ...
            if (list.length === 0) {
              ...
              list = allPartners.filter((p: any) => 
                p.type === 'transporter' || p.type === 'transporters' || p.type === 'haulier' || p.type === 'hauliers' ||
                p.category === 'transporter' || p.category === 'haulier' || p.role === 'transporter'
              );
            }
