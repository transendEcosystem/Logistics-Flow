...
async function performAdminAction(token: string, action: string, payload: any) {
    ...
    if (!response.ok || !result.success) {
        throw new Error(result.error || result.warning || `API Error for action: ${action}`);
    }
    return result;
}
        ...
        <DialogContent className="sm:max-w-lg">
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
                    ...
                    <FormField control={form.control} name="status" render={({ field }) => ( <FormItem><FormLabel>Status</FormLabel><Select onValueChange={field.onChange} value={field.value} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent><SelectItem value="active">Active</SelectItem><SelectItem value="inactive">Inactive</SelectItem></SelectContent></Select><FormMessage /></FormItem> )} />
...
export default function PartnerManagement() {
  ...
  const forceRefresh = useCallback(async () => {
    ...
    try {
        ...
        const result = await performAdminAction(token, 'searchRegistry', { type: 'partner' });
    ...
    if (type === 'invite' && data) {
        try {
            ...
            await performAdminAction(token, 'savePartner', { 
                partner: { ...data, invitationStatus: 'invited' } 
            });
            forceRefresh();
  ...
  const handleDelete = async () => {
    ...
    try {
        const token = await getClientSideAuthToken();
        if (!token) throw new Error("Authentication failed.");
        await performAdminAction(token, 'deletePartner', { partnerId: dialogState.data.id, source: dialogState.data.source });
